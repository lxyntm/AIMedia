    ...
    def _run(self, query: str) -> str:
        ...
        try:
            if selected_model == 'glm':
                ...
                response = llm.invoke(
                    f"""请从{raw_content}中提取关键信息，按照以下格式输出：
                        时间：[具体发生时间，不允许添加或者改变，严重按照原文提取]
                        地点：[事件发生地点]
                        人物：[相关人物及身份]
                        事件：[主要事件经过，200字以内]
                        背景：[相关背景信息，100字以内，保留信息日期和出处，类似格式：据参考消息援引韩联社1月13日报道]
                        影响：[事件的社会影响，100字以内]
                        痛点：[事件背后相关痛点，100字以内]
                        原话：[人物的直接引述内容，使用原文引号，若有多处请用分号分隔，格式：人物1：“引述1”；人物2：“引述2”，200字以内]
                        概要：[内容概括，200字以内]
                ...
                print('other')
                # 使用 LLM 提取结构化信息
                prompt = f"""请根据用户输入提取关键信息，按照以下格式输出：
                                   时间：[具体发生时间，不允许添加或者改变，严重按照原文提取]
                                   地点：[事件发生地点]
                                   人物：[相关人物及身份]
                                   事件：[主要事件经过，200字以内]
...
# qurey = '朝鲜人民军特种作战部队战斗人员参与训练 图据视觉中国'
...
#
# # 添加文档
# success = kb.add_document("这是新文档的内容", "李亚鹏卖酒 声称历经3年酝酿的＂拎壶冲＂最高1298元/瓶.txt")
# if success:
#     print("文档添加成功")

# # 删除文档
# success = kb.delete_document("李亚鹏卖酒 声称历经3年酝酿的＂拎壶冲＂最高1298元/瓶.txt")
# if success:
#     print("文档删除成功")
