...
def update_app(rar_path: str, app_dir: str):
    """
    解压更新包并更新应用程序
    ...
    try:
        ...
        logging.info(f"开始解压更新包: {rar_path}")
        # 解压RAR文件
        patoolib.extract_archive(rar_path, outdir=str(temp_dir))
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: updater.py <rar_path> <app_dir>")
        sys.exit(1)
        
    rar_path = sys.argv[1]
    app_dir = sys.argv[2]
    
    update_app(rar_path, app_dir)
